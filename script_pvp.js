...
socket.onopen = () => {
    ...
    const matchId = localStorage.getItem('matchId'); // Store matchId when joining

    console.log(`userID: ${userID}, matchID: ${matchId}`)

    if (userID && matchId) {
        socket.send(JSON.stringify({ type: 'identify', userID, matchId }));
    }
};
...
document.getElementById('run-code').addEventListener('click', async function () {
    ...
    if (success)
    {
        ...
        if (userId) {
            ...
            .then(data => console.log("Server Response:", data))  // Loging it
